...
public class WiFiDirectActivity extends Activity implements
	...
	private ServiceConnection serviceConn = new ServiceConnection() {
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			// ����DeviceDetailFragment ע�ᵽappNetService��
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bindService(new Intent(this, AppNetService.class), serviceConn,
				BIND_AUTO_CREATE);
		goHome = this.getIntent().getBooleanExtra("goHome", false);
		send = this.getIntent().getBooleanExtra("send", false);
		System.out.println("ActionBar : " + this.getActionBar());
	...
	private void verifyRecvFile() {
		...
		normalDia.setPositiveButton("Yes",
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
		...
		normalDia.setNegativeButton("No",
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						AppNetService netService = getNetService();
